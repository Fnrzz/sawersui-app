...
export default function CallbackPage() {
  ...
  const { isAuthenticated, isLoading, userAddress, needsRegistration, isCheckingProfile, loginToSupabase } = useZkLogin();
  const [showRegistration, setShowRegistration] = useState(false);
  const [isSyncing, setIsSyncing] = useState(false);

  useEffect(() => {
    // Wait for all checks to complete
    if (!isLoading && !isCheckingProfile && isAuthenticated) {
        if (needsRegistration) {
             ...
             const syncSession = async () => {
                 ...
                 if (result.success) {
                     const redirectPath = sessionStorage.getItem('zk_redirect_path') || "/dashboard";
  ...
  }, [isLoading, isAuthenticated, userAddress, needsRegistration, isCheckingProfile, router]); // remove isSyncing from deps to prevent loops
